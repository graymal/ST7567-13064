...
*/
ST7567::ST7567(int8_t mosi_pin,
               int8_t sclk_pin, int8_t dc_pin,
               int8_t rst_pin, int8_t cs_pin)
    : Adafruit_GFX(LCDWIDTH, LCDHEIGHT), spi(NULL), wire(NULL), buffer(NULL),
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
ST7567::ST7567(SPIClass *spi_ptr,
               int8_t dc_pin, int8_t rst_pin, int8_t cs_pin,
               uint32_t bitrate)
    : Adafruit_GFX(LCDWIDTH, LCDHEIGHT), spi(spi_ptr ? spi_ptr : &SPI), wire(NULL),
